...
function decideMode (preferBinary) {
	...
	// 	return 'ms-stream';
	// } else
	if (capability.arraybuffer && preferBinary) {
		return 'arraybuffer';
	} else {
		return 'text';
	}
...
ClientRequest.prototype._onFinish = function () {
	...
	if (opts.method !== 'GET' && opts.method !== 'HEAD') {
		body = new Blob(self._body, { type: (headersObj['content-type'] || {}).value || '' });
	}

	// create flattened list of headers
	const headersList = [];
	Object.keys(headersObj).forEach(keyName => {
		const { name } = headersObj[keyName];
		const { value } = headersObj[keyName];
